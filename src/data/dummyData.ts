...
// Generate dynamic show times based on current date
const generateShowTimes = () => {
  const today = new Date();
  const showTimes: Record<string, Array<{ time: string; showId: string }>> = {};
  ...
  for (let i = 0; i < 7; i++) {
    ...
    showTimes[dateStr] = [
      {
        time: new Date(date.setHours(10, 0, 0, 0)).toISOString(),
        showId: `show_${dateStr}_1`,
      },
      {
        time: new Date(date.setHours(14, 0, 0, 0)).toISOString(),
        showId: `show_${dateStr}_2`,
      },
      {
        time: new Date(date.setHours(18, 0, 0, 0)).toISOString(),
        showId: `show_${dateStr}_3`,
      },
      {
        time: new Date(date.setHours(21, 0, 0, 0)).toISOString(),
        showId: `show_${dateStr}_4`,
      },
    ];
  }
...
export const dummyBookingData = [
  ...
  },
  {
    ...
    show: {
      ...
      showDateTime: "2025-07-01T14:00:00.000Z",
      showPrice: 300,
    },
    amount: 600,
    bookedSeats: ["A1", "A2"],
...
// Some sample occupied seats for demo
export const dummyOccupiedSeats: Record<string, string[]> = {
  show_default: ["A1", "A2", "B5", "C3", "D7", "E10"],
};
